...
void FORB::writeBinary(const TDescriptor &descriptor, std::ofstream &f)
{
  f.write(descriptor.ptr<char>(), L);
}

FORB::TDescriptor FORB::readBinary(std::ifstream &f)
{
  cv::Mat A(1, L, CV_8U);
  f.read(reinterpret_cast<char*>(A.ptr()), L);
  return A;
}
