 ...
 * Raúl Mur-Artal
 *
 * Modification: Functionality for serialization & binary vocabulary handling.
 * Date: April 2016
 * Copyright (c) Univrses.
 */
...
public:
  ...
  /**
   * Saves the vocabulary into an archive.
   * @param archive
   *   cereal Archive.
   */
  ...
  /**
   * Loads the vocabulary from an archive.
   * @param archive
   *   cereal Archive.
   */
  template<class Archive>
...
protected:
  ...
  struct Node 
  {
    ...
    /**
     * Serializes the node from/to an archive.
     * @param archive
     *   cereal Archive.
     */
    ...
    {
      archive(id, weight, children, parent, descriptor, word_id);
    }
  };
      ...
      else
      {
        ...
        for(unsigned int c = 0; c < clusters.size(); ++c)
        {
          ...
          */
          ...
          if(clusters[c].empty()) { clusters.erase(clusters.begin() + c); }
        }
...
template<class TDescriptor, class F>
template<class Archive>
void TemplatedVocabulary<TDescriptor,F>::save(Archive &archive) const
{
  archive(m_k, m_L, m_weighting, m_scoring, m_nodes, m_words.size());
  for (auto &i : m_words)
    archive(i->id);
}
...
template<class TDescriptor, class F>
template<class Archive>
void TemplatedVocabulary<TDescriptor,F>::load(Archive &archive)
{
  typename std::vector<Node*>::size_type i, n;
  archive(m_k, m_L, m_weighting, m_scoring, m_nodes, n);
  m_words.resize(n);
  for (i=0 ; i<n ; i++)
  {
    NodeId id;
    archive(id);
    m_words[i] = &m_nodes[id];
  }
  createScoringObject();
}
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::saveBinary(const std::string &filename) const
{
  std::ofstream f;
  // Technically the 32-bit integer type is guaranteed to hold at least 32 bits
  // so the obfuscation of using a temporary variable below may not help with
  // portability, even though it is less likely to cause problems when moving
  ...
  while(!parents.empty())
  {
    ...
    for(pit = children.begin(); pit != children.end(); pit++, n++)
      if(!m_nodes[*pit].isLeaf())
  ...
  while(!parents.empty())
  {
    ...
    for(pit = children.begin(); pit != children.end(); pit++)
    {
      ...
      k = pid;
      f.write(reinterpret_cast<const char*>(&k), sizeof(k));
      f.write(reinterpret_cast<const char*>(&(child.weight)),
              sizeof(child.weight));
      F::writeBinary(child.descriptor, f);
  ...
  n = m_words.size();
  f.write(reinterpret_cast<const char*>(&n), sizeof(n));
  for ( typename std::vector<Node*>::const_iterator i = m_words.begin() ;
        i != m_words.end() ; i++)
  {
    k = (*i)->id;
    f.write(reinterpret_cast<const char*>(&k), sizeof(k));
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::loadBinary(const std::string &filename)
{
  m_words.clear();
  m_nodes.clear();
  ...
  f.open(filename.c_str(), std::ios::in | std::ios::binary);
    if(!f.good())
    {
      std::cerr << "could not load vocabulary file which is needed for running SLAM:" << filename << std::endl;
      std::abort();
    }
  ...
  m_L = k;
  f.read(reinterpret_cast<char*>(&k), sizeof(k));
  m_weighting = (WeightingType) k;
  f.read(reinterpret_cast<char*>(&k), sizeof(k));
  m_scoring = (ScoringType) k;
  createScoringObject();

  // Nodes.
  f.read(reinterpret_cast<char*>(&n), sizeof(n));
  m_nodes.resize(n+1); // +1 to include root
  for (size_t i = 0 ; i<n ; i++)
  {
    ...
    m_nodes[pid].children.push_back(nid);
    m_nodes[nid].descriptor = F::readBinary(f);
  }
