...
public:
  ...
  /**
   * Writes a descriptor to a binary stream.
   * @param descriptor
   * @param f Outout stream
   */
  static void writeBinary(const TDescriptor &descriptor, std::ofstream &f);

  /**
   * Reads a descriptor from a binary stream.
   * @param f Outout stream
   * @returns descriptor
   */
  static TDescriptor readBinary(std::ifstream &f);
};
